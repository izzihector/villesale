...
class MailMerge(object):
    ...
    def get_merge_fields(self, parts=None):
        ...
        for part in parts:
            for mf in part.findall('.//MergeField'):
                ...
                for mf in part.findall('.//MergeText'):
                    fields.add(mf.text.replace("MERGEFIELD",''))
        return fields

    def get_merge_fields_text(self, parts=None):
        if not parts:
            parts = self.parts.values()
        fields = []
        for part in parts:
            for mf in part.findall('.//MergeField'):
                for mf1 in mf.findall('.//MergeText'):
                    python_code = mf1.text.replace("MERGEFIELD",'').strip()[1:-1]
                    if "MERGEFORMAT" in mf1.text:
                        python_code=mf1.text.replace("MERGEFIELD",'').replace("MERGEFORMAT",'').replace("*",'').replace("\\",'').strip()
                    fields.append({
                        'name':mf.attrib['name'],
                        'text':python_code,
                        'text_origin':mf1.text
                        })
        return fields
    ...
    def __merge_field(self, part, field, text):
        ...
        for mf in part.findall('.//MergeField'):
            if mf.find('MergeText').text == field:
                children = list(mf)
