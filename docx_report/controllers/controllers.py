    ...
    def report_download(self, data, token):
        ...
        if type in ['qweb-docx']:
                ...
                if docids:
                    ...
                    # Particular report:
                    data_my = url_decode(url.split('?')[1]).items()  # decoding the args represented in JSON
                    response = self.report_routes(reportname, converter=converter, **dict(data_my))
                ...
                return response
        res = super(ReportControllerDocx, self).report_download(data, token)
        return res
